...
import mobi.sevenwinds.app.author.AuthorTable.createdAt
...
class BudgetEntity(id: EntityID<Int>) : IntEntity(id) {
    ...
    fun toYearResponse(): BudgetYearRecord {
        return BudgetYearRecord(
            year = year,
            month = month,
            amount = amount,
            type = type,
            author = authorId.let { AuthorEntity[it].name },
            createdAt = authorId.let { AuthorEntity[it].createdAt },
        )
    }
}